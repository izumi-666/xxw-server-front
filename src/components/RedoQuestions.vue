  ...
  <div class="assessment-container">
    ...
    <div class="assessment-header card">
      <div class="assessment-header-content">
        <h1 class="assessment-title">{{ formatExamName(assessmentInfo.exam_name) || "错题重做" }}</h1>
        ...
        <div class="modal-body">
          <div class="result-content">
            ...
            <div class="result-actions">
              ...
              <button class="btn-return" @click="returnToMistakesBook">返回错题本</button>
            </div>
          </div>
        </div>
      </div>
...
const loadAssessmentData = () => {
  try {
    // 从本地存储获取数据
    const assessmentDataStr = localStorage.getItem('current_assessment');
    
    if (!assessmentDataStr) {
      console.error('本地存储中未找到 current_assessment');
      ElMessage.error('未找到评测数据，请从错题本重新开始');
      setTimeout(() => {
    ...
    // 检查是否来自错题本
    if (data.from === 'mistakesbook') {
    }
    
    // 验证题目数据
    if (!data.questions) {
      console.warn('数据中没有 questions 字段，尝试查找可能的字段:', Object.keys(data));
      // 尝试查找可能的题目字段
      const possibleQuestionFields = ['questions', 'questionList', 'items', 'data'];
      for (const field of possibleQuestionFields) {
        if (data[field] && Array.isArray(data[field]) && data[field].length > 0) {
          data.questions = data[field];
          break;
        }
      }
    }
    ...
    if (!data.questions || !Array.isArray(data.questions) || data.questions.length === 0) {
      console.error('题目数据无效:', data.questions);
      throw new Error('题目数据为空或不合法');
    }
    ...
    assessmentInfo.value = {
      exam_name: data.exam_name || "错题重做练习",
      start_time: data.start_time || currentBeijingTime.toISOString(),
      duration: data.duration || 60,
      exam_id: data.exam_id || `exam_${Date.now()}`,
      paper_id: data.paper_id || `paper_${Date.now()}`,
      exam_history_id: data.exam_history_id || `history_${Date.now()}`,
    };
    
    // 处理题目数据 - 确保每个题目都有必要字段
    questions.value = data.questions.map((q, index) => {
      const questionId = q.id || q.question_id || `temp_${index}_${Date.now()}`;
      
      // 构建标准化的题目对象
      const standardizedQuestion = {
        id: questionId,
        title: q.title || q.content || `题目 ${index + 1}`,
        subject_id: q.subject_id || data.subject_id || 1,
        question_category_id: q.question_category_id || 1,
        marking_type: q.marking_type ?? 0, // 默认为选择题
        difficulty_level: q.difficulty_level || 2,
        options: q.options || {},
        answer: q.answer || '',
        img_url: q.img_url || '',
        knowledge_point_id: q.knowledge_point_id,
        sub_knowledge_point_ids: q.sub_knowledge_point_ids || [],
        score: q.score || 1,
        // 原始数据备份
        _raw: q
      };
      
      // 确保 options 是对象且有必要的结构
      if (!standardizedQuestion.options || typeof standardizedQuestion.options !== 'object') {
        standardizedQuestion.options = {};
      }
      
      // 如果没有选项但题目是选择题，提供默认选项
      if (standardizedQuestion.marking_type === 0 && 
          Object.keys(standardizedQuestion.options).length === 0) {
        standardizedQuestion.options = {
          option_A: "选项A",
          option_B: "选项B", 
          option_C: "选项C",
    ...
    // 初始化用户答案
    const savedAnswers = localStorage.getItem(`assessment_${assessmentInfo.value.exam_id}_answers`);
    if (savedAnswers) {
      try {
    ...
    // 初始化每个题目的答案
    questions.value.forEach(question => {
      if (question.id && !userAnswers.value[question.id]) {
        userAnswers.value[question.id] = '';
      }
    });
    
    // 初始化标记题目
    const savedMarks = localStorage.getItem(`assessment_${assessmentInfo.value.exam_id}_marks`);
    if (savedMarks) {
      try {
    ...
    // 设置计时器
    try {
      startTime.value = new Date(assessmentInfo.value.start_time);
      if (isNaN(startTime.value.getTime())) {
        console.warn('开始时间格式无效，使用当前时间');
        startTime.value = currentBeijingTime;
        assessmentInfo.value.start_time = startTime.value.toISOString();
      }
    } catch (error) {
    ...
    // 显示成功消息
    ElMessage.success({
      message: `已加载 ${questions.value.length} 道错题，可以开始练习`,
      duration: 3000
    });
    ...
    ElMessage.error({
      message: `加载评测数据失败: ${error.message}`,
      duration: 5000
    });
...
const returnToMistakesBook = () => {
  const userInfo = JSON.parse(localStorage.getItem("userInfo") || "{}");
  router.push({
    path: "/student/mistakesbook",
  });
};
